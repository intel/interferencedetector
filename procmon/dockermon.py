...
def get_args():
    ...
    parser.add_argument(
        "--collect_signatures",
        action="store_true",
        help="collect signatures of running containers and dump to: signatures.json",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Export collected data to cloudwatch. Requires setting the following parameters: aws_access_key_id, aws_secret_access_key, aws_region",
    )
    ...
    if args.export_to_cloudwatch and (
        args.aws_access_key_id is None
        or args.aws_secret_access_key is None
        or args.aws_region is None
    ):
        parser.error(
            "--export_to_cloudwatch requires --aws_access_key_id, --aws_secret_access_key, and --aws_region."
        )
    if args.cloudwatch_sampling_duration_in_sec < 1:
        parser.error(
            "Minimum value of --cloudwatch_sampling_duration_in_sec is 1 (one sample per second)"
        )

    return args


def initialize_cloudwatch_client(access_key_id, secret_acces_key, aws_region):
    try:
        client = boto3.client(
            "cloudwatch",
        ...
        print(
            "Failed to connect to cloudwatch client. ", err.response["Error"]["Message"]
        )
        sys.exit()
    return client
...
def send_to_cloud_watch(client, header, key, line):
    ...
    try:
        ...
        print("Failed to export data to cloudwatch. ", err.response["Error"]["Message"])
        sys.exit()
    return response
